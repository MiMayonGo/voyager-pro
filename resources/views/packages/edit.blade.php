<x-dashboard-layout title="Edit Package">
    <div class="max-w-2xl">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-xl font-bold text-gray-900">Edit Package</h1>
        ...
        <form method="POST" action="{{ route('packages.update', $package) }}" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('description', $package->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="available_slots" value="{{ old('available_slots', $package->available_slots) }}" min="0" required
                           class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                </div>
                <div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <label class="flex items-center gap-2 text-sm text-gray-700">
                        <input type="checkbox" name="categories[]" value="{{ $category->id }}"
                               {{ in_array($category->id, old('categories', $package->categories->pluck('id')->toArray())) ? 'checked' : '' }}>
                        {{ $category->name }}
                    </label>
                    @endforeach
                </div>
            ...
            <div>
                ...
                @if($package->cover_image)
                    <img src="{{ asset('storage/'.$package->cover_image) }}" class="h-20 rounded mb-2 object-cover">
                @endif
