<x-dashboard-layout title="Edit Package">

  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h5 class="fw-bold mb-0"><i class="bi bi-pencil-square me-2" style="color:#0F766E"></i>Edit Package</h5>
      <small class="text-muted">Update the details for <strong>{{ $package->title }}</strong></small>
    </div>
    <a href="{{ route('packages.show', $package) }}" class="btn btn-sm btn-outline-secondary">
          ...
          <div class="card-body">
            ...
            <div>
              ...
              <textarea name="description" rows="5"
                        ...
                        placeholder="Describe the tour experience, highlights, inclusions...">{{ old('description', $package->description) }}</textarea>
              ...
              <div class="col-sm-6">
                ...
                <select name="status" class="form-select @error('status') is-invalid @enderror">
                  @foreach(['draft','active','inactive'] as $s)
                    <option value="{{ $s }}" {{ old('status', $package->status) === $s ? 'selected' : '' }}>
                      {{ ucfirst($s) }}
                    </option>
                  @endforeach
                </select>
            ...
            @else
              <div class="d-flex flex-wrap gap-2">
                @foreach($categories as $category)
                  @php $checked = in_array($category->id, old('categories', $package->categories->pluck('id')->toArray())); @endphp
                  <div>
      ...
      <div class="col-12 col-xl-4">
        ...
        <div class="card border-0 shadow-sm mb-4">
          ...
          <div class="card-body">
            @if($package->cover_image)
              <div class="rounded-3 overflow-hidden mb-3" style="max-height:180px">
                <img src="{{ asset('storage/'.$package->cover_image) }}"
                     class="w-100" style="object-fit:cover;max-height:180px;display:block"
                     id="imgPreview">
              </div>
              <div class="text-muted mb-2" style="font-size:.75rem">
                <i class="bi bi-info-circle me-1"></i>Upload a new image to replace the current one.
              </div>
            @else
              <div class="rounded-3 d-flex align-items-center justify-content-center mb-3"
                   style="height:140px;background:#F8FAFC;border:2px dashed #CBD5E1" id="imgPreviewWrap">
                <div class="text-center text-muted">
                  <i class="bi bi-image fs-2 d-block mb-1"></i>
                  <span style="font-size:.78rem">No image yet</span>
                </div>
              </div>
            @endif
        ...
        <div class="card border-0 shadow-sm">
          <div class="card-body d-flex flex-column gap-2">
            ...
            <a href="{{ route('packages.show', $package) }}" class="btn btn-outline-secondary">
    ...
    document.getElementById('coverImageInput').addEventListener('change', function () {
      ...
      reader.onload = e => {
        ...
        if (!img) {
          ...
          img.id = 'imgPreview';
          img.className = 'w-100';
          img.style = 'object-fit:cover;max-height:180px;display:block';
